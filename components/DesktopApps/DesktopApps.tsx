...
const DesktopApps = (): JSX.Element => {
  ...
  useEffect(() => {
    const handleResize = () => {
      setPositions(prev => {
        ...
        const isOccupied = (targetX: number, targetY: number, excludeId: string) => {
          return Object.keys(next).some(
            id => id !== excludeId && next[id].x === targetX && next[id].y === targetY
          );
        };
  ...
  return (
    <Styled.DesktopContainer
      ...
      onMouseMove={(e) => {
        if (selectionBox?.isSelecting) {
          ...
          displayedDesktopApps.forEach((app) => {
             ...
             if (left < iconRight && right > iconLeft && top < iconBottom && bottom > iconTop) {
      ...
      onMouseUp={() => {
        if (selectionBox?.isSelecting) {
          setSelectionBox(prev => prev ? { ...prev, isSelecting: false } : null);
        }
      }}
      onMouseLeave={() => {
        if (selectionBox?.isSelecting) {
